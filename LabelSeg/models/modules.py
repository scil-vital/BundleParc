...
""" Modules for the Vision Transformer (ViT) for 3D data.
Most are taken from https://github.com/uni-medical/SAM-Med3D
"""
...
class LayerNorm3d(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.weight[:, None, None, None] * x + self.bias[
            :, None, None, None]
        return x
...
class DropPath(nn.Module):
    ...
    From https://github.com/fudan-zvg/SETR/blob/main/mmseg/models/backbones/vit.py  # noqa E501
    """
...
class Block3D(nn.Module):
    ...
    def _window_forward(self, x: torch.Tensor) -> torch.Tensor:
        norm = self.drop_path(self.norm1(x))
        # Window partition
        D, H, W = x.shape[1], x.shape[2], x.shape[3]
        part, pad_dhw = window_partition3D(norm, self.window_size)

        attn = self.drop_path(self.attn(part))
        # Reverse window partition
        unpart = window_unpartition3D(
            attn, self.window_size, pad_dhw, (D, H, W))
        x = x + self.drop_path(self.mlp(self.norm2(unpart)))

        return x

    def _global_forward(self, x: torch.Tensor) -> torch.Tensor:
...
class FlashAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        qkv = self.qkv(x).reshape(B, D * H * W, 3,
        ...
        attn = F.scaled_dot_product_attention(
            q, k, v, dropout_p=0.0)
        x = attn.view(
            B, self.num_heads, D, H, W, -1).permute(
                0, 2, 3, 4, 1, 5).reshape(B, D, H, W, -1)
        x = self.proj(x)
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if self.use_rel_pos:
            ...
            ), "Input size must be provided if using relative positional " + \
                "encoding."
            # initialize relative positional embeddings
            self.rel_pos_d = nn.Parameter(torch.zeros(
                2 * input_size[0] - 1, head_dim))
            self.rel_pos_h = nn.Parameter(torch.zeros(
                2 * input_size[1] - 1, head_dim))
            self.rel_pos_w = nn.Parameter(torch.zeros(
                2 * input_size[2] - 1, head_dim))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_rel_pos:
            attn = add_decomposed_rel_pos(
                attn, q, self.rel_pos_d, self.rel_pos_h,
                self.rel_pos_w, (D, H, W), (D, H, W))
...
def window_partition3D(x: torch.Tensor, window_size: int) -> Tuple[
    torch.Tensor, Tuple[int, int, int]
]:
    """
...
def window_unpartition3D(
    windows: torch.Tensor, window_size: int, pad_dhw: Tuple[int, int, int],
    dhw: Tuple[int, int, int]
) -> torch.Tensor:
    """
    Window unpartition into original sequences and removing padding.
    Args:
        windows (tensor): input tokens with [B * num_windows, window_size,
            window_size, C].
        window_size (int): window size.
    ...
    D, H, W = dhw
    B = windows.shape[0] // (
        Dp * Hp * Wp // window_size // window_size // window_size)
    x = windows.view(B, Dp // window_size, Hp // window_size,
                     Wp // window_size, window_size, window_size,
                     window_size, -1)
...
def add_decomposed_rel_pos(
    ...
    attn = (
        attn.view(
            B, q_d, q_h, q_w, k_d, k_h, k_w) + rel_d[:, :, :, :, None, None] +
        rel_h[:, :, :, None, :, None] + rel_w[:, :, :, None, None, :]
