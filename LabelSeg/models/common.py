# Taken from https://github.com/uni-medical/SAM-Med3D/blob/main/segment_anything/modeling/image_encoder3D.py  # noqa E501
import numpy as np
...
# References:
# Pos embed: https://github.com/facebookresearch/mae/blob/main/util/pos_embed.py
...
def get_3d_sincos_pos_embed_from_grid(embed_dim, grid):
    assert embed_dim % 3 == 0, (embed_dim // 3, embed_dim % 3)

    # use half of dimensions to encode grid_h
    emb_h = get_1d_sincos_pos_embed_from_grid(
        embed_dim // 3, grid[0])  # (H*W*D, C/3)
    emb_w = get_1d_sincos_pos_embed_from_grid(
        embed_dim // 3, grid[1])  # (H*W*D, C/3)
    emb_d = get_1d_sincos_pos_embed_from_grid(
        embed_dim // 3, grid[2])  # (H*W*D, C/3)
...
def get_1d_sincos_pos_embed_from_grid(embed_dim, pos):
    ...
    pos = pos.reshape(-1)  # (M,)
    out = np.einsum('m,d->md', pos, omega)  # (M, D/3), outer product

    emb_sin = np.sin(out)  # (M, D/3)
    emb_cos = np.cos(out)  # (M, D/3)
    ...
    def __init__(
        ...
        Parameters:
            ...
            act_layer (nn.Module):
                Activation layer.
            use_rel_pos (bool):
                If True, add relative positional embeddings to
                the attention map.
            rel_pos_zero_init (bool):
                If True, zero initialize relative
                positional parameters.
            window_size (int):
                Window size for window attention blocks. If it equals 0, then
                use global attention.
            input_size (tuple(int, int) or None):
                Input resolution for calculating the relative
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        -----------
            ...
            qkv_bias (bool):  If True, add a learnable bias to query, key, val.
        ...
        if self.use_rel_pos:
            ...
            self.rel_pos_w = nn.Parameter(
                torch.zeros(2 * input_size[2] - 1, head_dim))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, D, H, W, _ = x.shape
        # qkv with shape (3, B, nHead, H * W, C)
        qkv = self.qkv(x).reshape(B, D * H * W, 3, self.num_heads, -1).permute(
            2, 0, 3, 1, 4)
        ...
        if self.use_rel_pos:
            attn = add_decomposed_rel_pos(
                attn, q, self.rel_pos_d, self.rel_pos_h, self.rel_pos_w,
                (D, H, W), (D, H, W))

        attn = attn.softmax(dim=-1)
        x = (attn @ v).view(B, self.num_heads, D, H, W, -1).permute(
...
def window_partition3D(
    ...
    -------
        windows:
            windows after partition with [B * num_windows, window_size,
            window_size, C].
        (Hp, Wp):
